...
export class SlicerOptionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.slicerOptionService
      ...
      .subscribe((res: HttpResponse<ISlicerOption[]>) => this.paginateSlicerOptions(res.body, res.headers));
  }
  ...
  trackId(index: number, item: ISlicerOption) {
    return item.id;
  }
  ...
  protected paginateSlicerOptions(data: ISlicerOption[], headers: HttpHeaders) {
